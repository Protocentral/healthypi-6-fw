 ...
 * Copyright (c) 2026 Protocentral Electronics (I2C fork)
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * Local I2C fork of the upstream bosch,bmi323 driver. Upstream only ships
 * an SPI bus backend; this fork swaps in an I2C backend (bmi323_i2c.c)
 * while reusing the upstream logic unchanged. The fork uses the
 * "protocentral,bmi323-i2c" compatible so it doesn't clash if upstream
 * later grows its own I2C support.
 */
...
LOG_MODULE_REGISTER(bmi323_i2c);

#define DT_DRV_COMPAT protocentral_bmi323_i2c
...
static int bosch_bmi323_init_irq(const struct device *dev)
{
	...
	/* Fork: int-gpios is optional in the I2C binding. Triggers become a
	 * no-op when unwired; continue init without them.
	 */
...
/* Fork: I2C-only bus backend. */
#define BMI323_DEVICE_BUS(inst)                                                                    \
	BUILD_ASSERT(DT_INST_ON_BUS(inst, i2c), "protocentral,bmi323-i2c requires an I2C bus");    \
